...
class Enemy {
  ...
  constructor(theRoot, enemySpot, type) {
    ...
    this.x =
      type === "fin" ? enemySpot * FINENEMY_WIDTH + 15 : -FINENEMY_WIDTH - 100;
    ...
    this.y =
      type === "fin"
        ? -FINENEMY_HEIGHT - 20
        : 470 - enemySpot * BODYENEMY_HEIGHT;
    this.destroyed = false;
    this.isOutsideBox = false;
    ...
    this.domElement.src =
      type === "fin"
        ? "./images/water/sharkfin.gif"
        : "./images/water/sharkbody.gif";
    // We modify the CSS style of the DOM node.
    // this.domElement.style.objectFit = "fill";
    ...
    this.domElement.style.width =
      type === "fin" ? `${FINENEMY_WIDTH - 30}` : `${BODYENEMY_WIDTH}`;
    this.domElement.style.height =
      type === "fin" ? `300px` : `${PLAYER_HEIGHT}px`;
    type === "body" && (this.domElement.style.transform = "rotate(180deg)");
    type === "body" && (this.domElement.style.opacity = "0.4");
    this.domElement.style.zIndex = "5";

    // Show that the user can actually see the img DOM node, we append it to the root DOM node.
    theRoot.appendChild(this.domElement);
    this.speed = Math.random() / 2 + 0.18;
  }
  ...
  update(timeDiff) {
    ...
    this.y = this.type === "fin" ? this.y + timeDiff * this.speed : this.y;
    this.domElement.style.top = `${this.y}px`;

    this.x =
      this.type === "fin" ? this.x : this.x + (timeDiff * this.speed) / 10;
    this.domElement.style.left = `${this.x}px`;
    ...
    if (this.type === "fin") {
      if (this.y + 2 * FINENEMY_HEIGHT - 80 > GAME_HEIGHT) {
        this.destroyed = true;
        // this.domElement.style.border = "1px solid orange";
      } else if (this.isOutsideBox) {
